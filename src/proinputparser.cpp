...
uint8_t Parser::buttons_bit_position(BUTTONS button) {
  switch (button) {
  case Y:
  case minus:
    return 0x01;
...
uint8_t Parser::buttons_byte_button_value(BUTTONS button) {
  switch (button) {
  case Y:
  case minus:
    return 0x01;
...
size_t Parser::buttons_data_address(BUTTONS button, PacketType packet) {
...
size_t Parser::axis_data_address_high(AXIS axis, PacketType packet) {
...
size_t Parser::axis_data_address_low(AXIS axis, PacketType packet) {
  std::array<size_t, 4> address;
  switch (packet) {
  case PacketType::standard_input_report:
    address[0x00] = 0x06;
...
uint8_t Parser::dpad_bit_position(DPAD dpad) {
  switch (dpad) {
  case d_left:
    return 0x04;
  case d_right:
...
uint8_t Parser::dpad_byte_value(DPAD dpad) {
  switch (dpad) {
  case d_left:
    return 0x08;
  case d_right:
...
size_t Parser::dpad_data_address(DPAD dpad, PacketType packet) {
...
void Parser::print_exchange_array(HidApi::default_packet arr, PacketType type) {
  bool redcol = false;
  if (arr[0] != 0x30) {
    //printColor::yellow();
  }
...
const char *Parser::button_name(BUTTONS button) {
  switch (button) {
  case A:
    return "A";
  case B:
...
const char *Parser::axis_name(AXIS axis) {
  switch (axis) {
  case axis_lx:
    return "axis_lx";
  case axis_ly:
...
const char *Parser::dpad_name(DPAD dpad) {
  switch (dpad) {
  case d_left:
    return "d_left";
  case d_right:
...
const std::array<ProInputParser::BUTTONS, 14> Parser::btns_ids = {
...
const std::array<ProInputParser::AXIS, 4> Parser::axis_ids = {
  ProInputParser::axis_lx, ProInputParser::axis_ly, ProInputParser::axis_rx, ProInputParser::axis_ry,
};

const std::array<ProInputParser::DPAD, 4> Parser::dpad_ids = {
